...
var story_editing = (function () {
    ...
    function create_assertion_row(l_text, relation_text, r_text, index){
        //out frame
		...
		$("#"+new_frame.id).css({
			top: 0+"px",
			left: Math.floor(parent_frame_width/3*0)+"px",
			position:'absolute',
		...
		$("#"+new_frame.id).css({
			top: 0+"px",
			left: Math.floor(parent_frame_width/3*1)+"px",
			position:'absolute',
			height: (each_story_settings.assertion_frame_hieght-2)+"px",
            width: Math.floor(parent_frame_width/3-5)+"px",
            zIndex: 5,
		...
		$("#"+new_frame.id).css({
			top: 0+"px",
			left: Math.floor(parent_frame_width/3*2)+"px",
			position:'absolute',
			height: (each_story_settings.assertion_frame_hieght-2)+"px",
            width: Math.floor(parent_frame_width/3-5)+"px",
            zIndex: 5,
    ...
    var initialize_editing_interface = function() {
        ...
        for(var item in each_story_settings.assert_pool){
            ...
            //console.log(item);
            create_assertion_row(each_story_settings.assert_pool[item]['r'], each_story_settings.assert_pool[item]['relation'], each_story_settings.assert_pool[item]['l'], item);
        }
